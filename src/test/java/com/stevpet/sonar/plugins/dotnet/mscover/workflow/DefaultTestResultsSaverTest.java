...
public class DefaultTestResultsSaverTest {
	...
	public void before() {
		...
		testResultsSaver = new DefaultTestResultsSaver(sensorContext,resourceResolver,testResultsFormatter);
	...
	public void oneResults_oneFormatted() {
		...
		testResultsSaver.save(projectUnitTestResults);
		verify(testResultsFormatter,times(1)).formatClassUnitTestResults(any(ClassUnitTestResult.class));
	}
	...
	public void oneResults_metricsSavedSaved() {
		...
		when(resourceResolver.getFile(any(File.class))).thenReturn(org.sonar.api.resources.File.create("bogus"));
		testResultsSaver.save(projectUnitTestResults);
		verify(sensorContext,times(6)).saveMeasure(any(Resource.class),any(Metric.class), anyDouble());
	}
	@Test
	public void twoResults_twoSaved() {
		...
		when(resourceResolver.getFile(any(File.class))).thenReturn(org.sonar.api.resources.File.create("bogus"));
		testResultsSaver.save(projectUnitTestResults);
		verify(testResultsFormatter,times(2)).formatClassUnitTestResults(any(ClassUnitTestResult.class));
	}
	...
	public void oneResults_notFound() {
		...
		testResultsSaver.save(projectUnitTestResults);
		verify(testResultsFormatter,times(0)).formatClassUnitTestResults(any(ClassUnitTestResult.class));
	}
}
